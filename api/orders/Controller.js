...
const addOrders = async (req, res) => {
    ...
    const { customerEmail, items, totalBill, customerAddresss, customerContact, customerName } = req.body;
    ...
    if (!customerEmail || !items || !totalBill || !customerAddresss || !customerContact || !customerName) {
        ...
        try {
            ...
            const order = await orderDB.create({ id: getId.seq, customerEmail, items, totalBill, customerAddresss, customerContact, customerName })
...
const updateOrder = async (req, res) => {
    ...
    const { _id, customerName, status, customerEmail, customerAddresss, customerContact } = req.body;

    if (!_id || !customerName || !status || !customerEmail || !customerAddresss || !customerContact) {
        res.json({
        ...
        try {
            const filter = { _id }
            const update = { customerName, status, customerEmail, customerAddresss, customerContact }
