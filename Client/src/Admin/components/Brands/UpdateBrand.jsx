...
function UpdateBrand({ getBrand, newBrand }) {
    const [show, setShow] = useState(false);
    const [brandId, setBrandId] = useState(getBrand.id)
    const [brandName, setBrandName] = useState(getBrand.BrandName);
    const [brandImage, setBrandImage] = useState(getBrand.BrandImage);
    const [brandCategory, setBrandCategory] = useState(getBrand.Category)
            ...
            uploadBytes(storageRef, brandImage).then((snapshot) => {
                ...
                getDownloadURL(snapshot.ref)
                    .then((url) => {
                        ...
                        const payload = { id: brandId, Name: brandName, Image: check ? url : brandImage, Category: brandCategory }

                        // Connect to our backend
                        axios.put(`/api/brand/update-brand`, payload)
                            .then(json => {
                                newBrand(json.data.brands)
                                setShow(false)
                            })
                            .catch(err => {
                                console.log(err)
            ...
            >
                ...
                <Modal.Body>
                    <form className="brand-form" onSubmit={handleSubmit}>
                        <p className="brand-form-title">Update Brand</p>
                        ...
                        <div className='brand-file'>
                            <label className="custom-file-upload m-4 p-4" htmlFor="file" style={{ cursor: 'pointer', border: "2px dashed #cacaca", height: '100px' }}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => (
                                        e.target.files[0] ? (setBrandImage(e.target.files[0]), setCheck(true)) : setCheck(false)
                                    )}
                                />
                            </label>
                            <h4 className='ms-5 mb-4'>{check ? brandImage.name : null}</h4>
                        </div>
